<p><div class ="Welcome"><h2>Welcome <?php echo $_SESSION['FirstName'] . " " . $_SESSION['LastName']?></h2></p><p>$
<?php
...
$select1 = $conn->prepare("SELECT major.M_Name FROM major,undergraduate WHERE major.Major_ID = undergraduate.Major$
...
$select2 = $conn->prepare("SELECT minor.MinorName FROM minor, undergraduate WHERE minor.Minor_ID = undergraduate.M$
...
if (isset($_SESSION['undergradptid'])){
        ...
        $select4 = $conn->prepare("SELECT Credits_Num FROM undergradfulltime WHERE undergradfulltime.u_undergradfu$
...
$select5 = $conn->prepare("SELECT course.C_Name FROM history,section,course  WHERE history.Stud_ID = ? AND section$
$select5->bindParam(1,$_SESSION['user_id']);
$course = $select5->execute();
$courses =  $select5->fetchALL(PDO::FETCH_COLUMN,0);
//echo $course[0];

$select6 = $conn->prepare("SELECT first_name FROM history,section,faculty,person WHERE history.Stud_ID = ? AND sec$
